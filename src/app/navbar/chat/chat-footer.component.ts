...
export class ChatFooterComponent implements OnInit, OnDestroy {
  ...
  public viewMore($event: Event, id: string): void {
    if ($event.srcElement.scrollTop < 20 && !this.processing) {
      this.processing = true;
      const page: string = $event.srcElement.attributes.getNamedItem('data-page').value;
      const newPage = Number(page) + 1;
      this.chatService.viewMore(id, newPage).then(() => {
        $event.srcElement.setAttribute('data-page', newPage.toString());
